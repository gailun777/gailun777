...
export function pickRiskLevel({ spreadBps, quoteVolume, depthNotional, fundingRate, canOpenWith1U20x }) {
  if (!canOpenWith1U20x || spreadBps > 10 || depthNotional < scannerConfig.minDepthNotionalUSDT / 2) return 'red';
  if (Math.abs(fundingRate) > 0.001 || spreadBps > scannerConfig.maxSpreadBps || quoteVolume < scannerConfig.minQuoteVolumeUSDT) return 'yellow';
  return 'green';
}

async function gateGet(path) {
  const resp = await fetch(`${GATE_API}${path}`, { headers: { Accept: 'application/json' } });
  if (!resp.ok) throw new Error(`gate_api_error ${resp.status} ${path}`);
  return resp.json();
}

export async function fetchGateMarketRows() {
  ...
  const selectedContracts = usdtContracts.slice(0, 60);
  ...
  return selectedContracts
    .map(c => {
      ...
      const openCheck = canOpenWith1UMargin(c, lastPrice, scannerConfig.maxLeverageCheck);
      const riskLevel = pickRiskLevel({ spreadBps, quoteVolume, depthNotional, fundingRate, canOpenWith1U20x: openCheck.canOpen });
      const inPool = quoteVolume >= scannerConfig.minQuoteVolumeUSDT && spreadBps <= scannerConfig.maxSpreadBps && depthNotional >= scannerConfig.minDepthNotionalUSDT && openCheck.canOpen;
      return {
        symbol: c.name,
        price: lastPrice,
...
export async function runScan() {
  return {
    ...
    killSwitch: false,
    rows: await fetchGateMarketRows(),
